...
const QuizPlatform = () => {
  ...
  const AdministratorView = () => {
    ...
    const [quizForm, setQuizForm] = useState({ name: '', duration: 30, questionCount: 10, subject: '', level: 'medium', questionSetId: '' });
      ...
      <div className="space-y-6">
        ...
        {view === 'questions' && (
          <div className="bg-white rounded-lg shadow p-6">
            ...
            {/* Show created question sets (label) */}
                ...
                <div className="space-y-2">
                  {myQuestionSets.map(qs => (
                    <div key={qs.id} className="p-2 border rounded flex justify-between items-center">
                      <div>
                        <div className="font-medium">{qs.label}</div>
                        <div className="text-sm text-gray-600">{qs.questionIds.length} question(s) — {qs.level} — {qs.topics?.join(', ')}</div>
                      </div>
                      <div className="flex gap-2 items-center">
                        <button onClick={() => {
                          const filtered = questions.filter(q => qs.questionIds.includes(q.id));
                          alert(`Questions in set "${qs.label}":\n` + filtered.map((q,i)=>`${i+1}. ${q.question}`).join('\n'));
              ...
              <button
                onClick={() => {
                  if (quizForm.name && quizForm.subject) {
                    createQuiz(quizForm.name, quizForm.duration, quizForm.questionCount, quizForm.subject, quizForm.level, [], quizForm.questionSetId || undefined);
                    setQuizForm({ name: '', duration: 30, questionCount: 10, subject: '', level: 'medium', questionSetId: '' });
                  }
                }}
            ...
            <div className="space-y-2">
              {myQuizzes.map(q => (
                <div key={q.id} className="p-3 border rounded">
                  ...
                  <p className="text-sm text-gray-600">Duration: {q.duration} min | Questions: {q.questionCount} | Subject: {q.subject} | Level: {q.level} {q.questionSetId ? `| Set: ${questionSets.find(s=>s.id===q.questionSetId)?.label}` : ''}</p>
                </div>
              ))}
            </div>
          </div>
                  ...
                  <div key={b.id} className="p-3 border rounded">
                    <div className="flex justify-between items-start mb-2">
                      ...
                      <div className="flex items-center gap-2">
                        ...
                        <button onClick={() => deleteBatch(b.id)} className="px-3 py-1 rounded text-sm bg-red-600 text-white hover:bg-red-700">Delete</button>
                      </div>
                    </div>
                          ...
                          return (
                            <div key={s.id} className="flex justify-between items-center p-2 bg-gray-50 rounded">
                              ...
                              <div className="flex items-center gap-2">
                                ...
                                {attempt && (
                                  <button onClick={() => { setCurrentAttempt(attempt); setCurrentView('student-review'); }} className="text-sm text-blue-600 hover:underline">Review</button>
                                )}
                              </div>
                            </div>
                          );
                  ...
                  <div key={s.id} className="p-3 border rounded">
                    <div className="flex items-center justify-between gap-4 mb-2">
                      ...
                      <div className="flex items-center gap-2">
                        ...
                        <button onClick={() => { if (confirm('Delete this supervisor?')) deleteSupervisor(s.id); }} className="px-3 py-1 rounded text-sm bg-red-600 text-white hover:bg-red-700"><Trash2 size={16} /></button>
                      </div>
                    </div>
                    ...
                    {attempts.map(a => (
                      <div key={a.id} className="flex justify-between items-center p-2 bg-gray-50 rounded">
                        ...
                        <div className="flex items-center gap-2">
                          ...
                          <button onClick={() => { setCurrentAttempt(a); setCurrentView('student-review'); }} className="text-sm text-blue-600 hover:underline">Review</button>
                        </div>
                      </div>
                    ))}
                  </div>
                      ...
                      {batchStudents.map(s => (
                        <div key={s.id} className="text-sm p-2 bg-gray-50 rounded flex justify-between items-center">
                          ...
                          <div className="flex items-center gap-2">
                            ...
                            {batch.activeQuizId && (() => {
                              const attempt = getLatestFinishedAttempt(s.id, batch.activeQuizId, batch.id);
                              return attempt ? <button onClick={() => { setCurrentAttempt(attempt); setCurrentView('student-review'); }} className="text-sm text-blue-600 hover:underline">Review</button> : null;
                            })()}
                          </div>
                        </div>
                      ))}
