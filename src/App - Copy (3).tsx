...
const QuizPlatform = () => {
  ...
  const [questionSets, setQuestionSets] = useState<any[]>([]); // NEW: question sets created via CSV import
...
const uploadQuestionsFromFile = async (file: File, subject: string, topicsString: string, level: string) => {
  ...
  const [batchAddSelections, setBatchAddSelections] = useState<Record<string,string>>({}); // for adding existing students to specific batch
  ...
  {/* Drag & drop area spanning full width under inputs */}
  <div
    className="col-span-1 md:col-span-3 p-3 border-dashed border-2 rounded border-gray-300 text-sm text-gray-600 flex items-center justify-center"
  ...
  >
    Drop CSV file here (one question per line). Format:
    <div className="ml-2 font-mono text-xs">
      &lt;QuestionNumber&gt;@@&lt;QUESTION&gt;@@&lt;Option1||Option2||Option3||Option4&gt;@@&lt;Answer (text or 1-based index)&gt;
    </div>
  </div>
</div>
              ...
              <div className="space-y-2">
                {myQuestionSets.map(qs => (
                  <div key={qs.id} className="p-2 border rounded flex justify-between items-center">
                    ...
                    <div className="flex gap-2 items-center">
                      <button onClick={() => {
                        // show questions in this set by filtering the view (quick approach)
                        const filtered = questions.filter(q => qs.questionIds.includes(q.id));
                        // set a temporary view: we simply alert a quick summary or you can implement a modal
